...
ETF <- c("QQQ", "XLK", "SPY", "XLF", "ARKK", "WCLD", "IGV", "CSCO", "TSLA", "AMD")
...
# le caclulo t+1, t+2 y t+3
 ...
 # le filtro puntos extremos definidos arriba y abajo de 20%
...
# poner un control porque ETF puede tener 

tabla <- bind_cols(tabla, rep(0, nrow(tabla)),rep(0, nrow(tabla)), rep(0, nrow(tabla)),rep(0, nrow(tabla)),rep(0, nrow(tabla)),rep(0, nrow(tabla)),rep(0, nrow(tabla)),rep(0, nrow(tabla)),rep(0, nrow(tabla)),rep(0, nrow(tabla)),rep(0, nrow(tabla)) )
...
tabla %>% 
  ...
  kable_styling(bootstrap_options = "striped", full_width = F) %>% 
  add_header_above(c(" " = 4, "Retorno en T+0 > 0" = 4, "Retorno en T+0 < 0" = 4))
````
<br>
